...
def api_tickets_bulk():
    """
    Bulk ticket actions (complete selected tickets).

    Request body (JSON):
        {
            "action": "complete",
            "ticket_ids": ["ACT-...", "ACT-..."],
            "notes": "Bulk triage notes (optional)"
        }

    Returns:
        {
            "success": true,
            "results": [{"id": "ACT-...", "success": true}, ...]
        }
    """
    # Require authentication for mutations
    if not _require_auth(request):
    ...
    if action != 'complete':
        return jsonify({'error': 'Unsupported action. Only "complete" supported.'}), 400

    ticket_ids = data.get('ticket_ids', [])
    if not ticket_ids or not isinstance(ticket_ids, list):
        return jsonify({'error': 'ticket_ids array required'}), 400

    notes = data.get('notes', 'Bulk completed via dashboard triage UI (ACT-20260125-BD520)')
    ...
    for tid in ticket_ids:
        try:
            success = mark_ticket_complete(
                ...
                test_steps='Bulk UI verification: tickets selected and completed together',
                test_results='Confirmed in dashboard: tickets marked completed after bulk action',
                summary=f'Bulk triage complete ({len(ticket_ids)} tickets)',
                paths=paths,
            )
            results.append({'id': tid, 'success': bool(success)})
            emit_ticket_event('ticket_completed', {
                'ticket_id': tid,
                'timestamp': datetime.now(timezone.utc).isoformat(),
            })
