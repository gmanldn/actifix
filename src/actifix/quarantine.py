...
def validate_ticket_block(block: str) -> tuple[bool, str]:
    """
    Validate a ticket block structure.
    
    Args:
        block: Ticket block content.
    ...
    if "### ACT-" not in block:
        errors.append("Missing ticket ID header (### ACT-...)")
    
    if "**Priority**:" not in block:
        errors.append("Missing Priority field")
    
    if "**Error Type**:" not in block:
        errors.append("Missing Error Type field")
    
    if "**Source**:" not in block:
        errors.append("Missing Source field")
    
    if "**Checklist:**" not in block and "- [" not in block:
...
def repair_list_file(
    paths: Optional[ActifixPaths] = None,
) -> tuple[int, int]:
    """
    Repair ACTIFIX-LIST.md by quarantining malformed tickets.
    
    Args:
        paths: Optional paths override.
    
    Returns:
        Tuple of (valid_count, quarantined_count).
    """
    if paths is None:
        paths = get_actifix_paths()
    ...
    for block in blocks:
        ...
        if block.startswith("### ACT-"):
            ...
            if is_valid:
                ...
                # Quarantine invalid block
                quarantine_content(
                    content=block,
                    source="ACTIFIX-LIST.md",
                    reason=f"Malformed ticket: {error}",
                    paths=paths,
                )
                quarantined_count += 1
        else:
    ...
    if quarantined_count > 0:
        ...
        log_event(
            paths.aflog_file,
            "LIST_FILE_REPAIRED",
            f"Repaired list file: {quarantined_count} malformed tickets quarantined",
            extra={"valid": len(valid_blocks), "quarantined": quarantined_count}
        )
    
    return len(valid_blocks), quarantined_count
