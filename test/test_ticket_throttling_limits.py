...
from actifix.raise_af import ActifixEntry, TicketPriority, record_error, generate_duplicate_guard
...
def repo(test_config):
    ...
    repo = TicketRepository(pool=pool, config=test_config)
    yield repo
    reset_database_pool()
    reset_ticket_repository()
...
class TestTicket2FileContextSize:
    ...
    def test_create_ticket_file_context_exceeds_limit(self, repo):
        """Create ticket with file_context exceeding limit (should raise FieldLengthError)."""
        ...
        # Should raise FieldLengthError for file context exceeding limit
        with pytest.raises(FieldLengthError) as excinfo:
            repo.create_ticket(entry)
        assert "file_context" in str(excinfo.value).lower()
        assert "1048576" in str(excinfo.value)  # 1MB limit
...
class TestTicket3OpenTicketsLimit:
    ...
    def test_create_multiple_tickets_within_limit(self, repo):
        ...
        for i in range(5):
            ...
            entry = ActifixEntry(
                ...
                source="test/test_runner.py:42",
                run_label="test",
                entry_id=f"ACT-20260114-T3-{i:03d}",
                created_at=datetime.now(timezone.utc),
                priority=TicketPriority.P2,
                error_type="TestError",
                duplicate_guard=generate_duplicate_guard("test/test_runner.py:42", message, "TestError"),
            )
            assert repo.create_ticket(entry) is True
    ...
    def test_open_tickets_count_excludes_completed(self, repo):
        ...
        # Create 2 tickets
        for i in range(2):
            message = f"Test completed ticket {i}"
            entry = ActifixEntry(
                message=message,
                source="test/test_runner.py:42",
                run_label="test",
        ...
        repo.mark_complete(
            "ACT-20260114-100",
            "Fixed the issue by adding validation checks and defensive programming patterns",
            "Ran unit tests with pytest -v",
    ...
    def test_open_tickets_count_excludes_deleted(self, repo):
        ...
        # Create 2 tickets
        for i in range(2):
            message = f"Test deleted ticket {i}"
            entry = ActifixEntry(
                message=message,
                source="test/test_runner.py:42",
                run_label="test",
