...
class TestValidateTicketBlock:
    """Test ticket block validation."""
    
    def test_validate_ticket_block_valid(self):
        """Test validation of valid ticket block."""
        block = """### ACT-001
**Priority**: P1
...
"""
        ...
        is_valid, error = validate_ticket_block(block)
        
        assert is_valid
        assert error == ""
    
    def test_validate_ticket_block_missing_id(self):
        """Test validation fails without ticket ID."""
        block = """**Priority**: P1
**Error Type**: TestError
**Source**: test.py:10
**Checklist:**
...
"""
        ...
        assert not is_valid
        assert "ticket id" in error.lower()
    
    def test_validate_ticket_block_missing_priority(self):
        """Test validation fails without priority."""
        block = """### ACT-001
**Error Type**: TestError
**Source**: test.py:10
**Checklist:**
...
"""
    ...
    def test_validate_ticket_block_missing_error_type(self):
        """Test validation fails without error type."""
        block = """### ACT-001
**Priority**: P1
**Source**: test.py:10
**Checklist:**
...
"""
        ...
        assert not is_valid
        assert "error type" in error.lower()
    
    def test_validate_ticket_block_missing_source(self):
...
"""
    ...
    def test_validate_ticket_block_missing_checklist(self):
        """Test validation fails without checklist."""
        block = """### ACT-001
**Priority**: P1
**Error Type**: TestError
**Source**: test.py:10
"""
        ...
        assert not is_valid
        assert "checklist" in error.lower()


class TestRepairListFile:
    """Test list file repair functionality."""
    
    def test_repair_list_file_no_file(self, tmp_path):
        """Test repair when list file doesn't exist."""
        paths = get_actifix_paths(project_root=tmp_path)
        init_actifix_files(paths)
        ...
        valid, quarantined = repair_list_file(paths)
        
        assert valid == 0
        assert quarantined == 0
    
    def test_repair_list_file_valid_content(self, tmp_path):
        """Test repair with all valid tickets."""
        paths = get_actifix_paths(project_root=tmp_path)
        init_actifix_files(paths)
        
        content = """# ACTIFIX-LIST
...
"""
    ...
    def test_repair_list_file_malformed_ticket(self, tmp_path):
        """Test repair quarantines malformed tickets."""
        paths = get_actifix_paths(project_root=tmp_path)
        init_actifix_files(paths)
...
"""
        ...
        # Check quarantine was created
        assert get_quarantine_count(paths) == 1
    
    def test_repair_list_file_rewrites_file(self, tmp_path):
        """Test repair rewrites list file without malformed tickets."""
        paths = get_actifix_paths(project_root=tmp_path)
        init_actifix_files(paths)
...
"""
        ...
        repair_list_file(paths)
        
        new_content = paths.list_file.read_text()
        assert "ACT-001" in new_content
        assert "ACT-002" in new_content
        assert "ACT-BAD" not in new_content
    
    def test_repair_list_file_logs_event(self, tmp_path):
        """Test that repair logs an event."""
        paths = get_actifix_paths(project_root=tmp_path)
        init_actifix_files(paths)
...
"""
        ...
        log_content = paths.aflog_file.read_text()
        assert "LIST_FILE_REPAIRED" in log_content
